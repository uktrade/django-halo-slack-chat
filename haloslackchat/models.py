...
class NotFoundError(Exception):
    """Raised when a HaloSlackChat instance was not found."""


class HaloSlackChat(models.Model):
    ...
    # The ID of the linked issue in Halo:
    # e.g. 3451
    ...
    def open(cls, channel_id, chat_id, ticket_id=None, opened=None):
        ...
        :param ticket_id: The optional Halo Ticket ID.

        :param opened: The optional datetime (default is UTC now).

        :returns: A HaloSlackChat instance.
    ...
    def resolve(cls, channel_id, chat_id, closed=None):
        ...
        :returns: The resolved HaloSlackChat instance.
    ...
    def open_issues(cls):
        ...
        :returns: An empty list or list of HaloSlackChat instances.
...
class CustomHeaderAdapter(requests.adapters.HTTPAdapter):
    """Allow custom request headers for Halo requests.
    """
    def add_headers(self, request, **kwargs):
        """Add in custom X-On-Behalf-Of for haloslackchat.

        This allows us to impersonate Haloslackchat directly so their
        email is assigned to the issue at the top level. Otherwise it
        seems the admin email gets used.

        For this to work the Halo scope 'impersonate' needs to be
        requested.

        """
        headers = request.headers
        headers['X-On-Behalf-Of'] = settings.HALO_AGENT_EMAIL
        request.headers = headers


class HaloApp(models.Model):
    """Used to store Halo OAuth client / app details after successfull
    completion of the OAuth process.
    ...
    @classmethod
    def client(cls):
        """Returns a Halo client instance ready for use.

        This recovers the latest HaloApp instance and uses its access_token
        field for the token.
        ...
        if settings.DEBUG:
            logging.getLogger(__name__).debug(
                f"Halo Access Token:{app.access_token}"
            )

        session = requests.Session()
        adapter = CustomHeaderAdapter(**{})#http_adapter_kwargs
