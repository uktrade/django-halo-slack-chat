"""
Functions used to aid in halo, slack, email messaging.
...
def message_issue_halo_url(
    slack_client, halo_uri, ticket_id, chat_id, channel_id
):
    """Post to slack where the Halo URL of the issue.
    """
...
def strip_halo_origin(text):
    # TODO
    # text = text.split('(Halo):')[-1].strip()
    # text = text.split('(From Halo Email):')[-1].strip()
    return text
...
def messages_for_slack(slack, halo):
    """Work out which messages from halo need to be added to the slack
    conversation.

    :param slack: A list of slack messages.

    :param halo: A list of halo comment message.
    ...
    for msg in slack:
        ...
        # Slack seems to use the name whereas halo uses the actual emoji:
        text = strip(msg['text'])
        # log.debug(
    ...
    for_slack = []
    for msg in halo:
        # Compare like with like, although this might not be needed on halo.
        # Apply the halo origin filter to prevent repeated email body
        # messages on slack.
        ...
        if msg['via']['channel'] == 'api':
            ...
            # for halo e.g. Messages for email user's not needed on slack.
        ...
        # else:
        #     log.debug(f"halo message not sent to slack:'{text}'")
