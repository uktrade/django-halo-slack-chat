...
from haloslackchat import views
...
@patch('haloslackchat.views.settings')
@patch('haloslackchat.views.requests.get')
...
def test_halo_oauth_missing_code():
    """Test handling if no code is present in the request.
    """
    factory = APIRequestFactory()
    request = factory.get('/halo/oauth/')
    response = views.halo_oauth(request)
    assert response.status_code == 400


@patch('haloslackchat.views.requests.post')
def test_halo_oauth_successful_token_exchange(
    requests_post, log, db
):
    """Test the ok path recoving the access token and creating the halo app 
    instance.
    ...
    # mock the OK response from Halo 
    requests_post.return_value.json = loads
    settings = dict(
        DEBUG = True,
        HALO_SUBDOMAIN = 'bob_town',
        HALO_REDIRECT_URI = 'https://my.app/oauth',
        HALO_CLIENT_IDENTIFIER = 'my-client-id',
        HALO_CLIENT_SECRET = 'my-client-secret',
    )

    factory = APIRequestFactory()
    request = factory.get('/halo/oauth/', dict(code=4321))
    with patch.dict('webapp.settings.__dict__', settings, clear=True):
        response = views.halo_oauth(request)

    # Verify the response and the newly configured HaloApp instance:
    assert response.status_code == 200
    assert HaloApp.objects.count() == 1
    app = HaloApp.objects.first()
