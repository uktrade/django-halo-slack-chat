...
def test_daily_summary_data(log, db):
    ...
    report  = HaloSlackChat.daily_summary(workspace_uri, when=now)
...
def test_daily_report_plaintext(log, db):
    ...
    plain_text  = HaloSlackChat.daily_report(report)
