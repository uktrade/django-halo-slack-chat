...
class FakeApi(object):
    """Aid testing tickets without using Halo API directly.
    """
    ...
    class FakeTicketCRUD(object):        
        ...
        def create(self, ticket):
            """Pretend to create a halo ticket and return the canned
            result.
            """
...
def test_halo_ticket_url(log):
    """Verify the URL generated to point at (UI not API) ticket in halo.
    """
    # check trailing and non-trailing slash set
    halo_ticket_url = 'https://example.com/agent/tickets'
    url = halo_api.halo_ticket_url(halo_ticket_url, '123')
    assert url == 'https://example.com/agent/tickets/123'

    halo_ticket_url = 'https://help.example.com/agent/tickets/'
    url = halo_api.halo_ticket_url(halo_ticket_url, '123')
    assert url == 'https://help.example.com/agent/tickets/123'

    # handle integer ticket ID being passed in e.g. ticket.id
    halo_ticket_url = 'https://frog.example.com/agent/tickets/'
    url = halo_api.halo_ticket_url(halo_ticket_url, 17)
...
def test_get_ticket_with_result(log):
    ...
    returned = halo_api.get_ticket(client, 12345)
...
def test_get_ticket_with_no_result(log):
    ...
    returned = halo_api.get_ticket(client, chat_id)

    assert returned is None


def test_create_ticket(log):
    """Test out the behaviour when 'creating' a halo ticket.
    """
...
def test_close_ticket(log):
    ...
    returned = halo_api.close_ticket(client, 12345)
