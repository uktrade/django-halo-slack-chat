...
@patch('haloslackchat.message_tools.post_message')
def test_message_issue_halo_url_tools(post_message, db, log):
    """Test slack message for the halo issue link.
    """
...
@patch('haloslackchat.message_tools.post_message')
@patch('haloslackchat.models.APISession.get')
...
def test_new_message_for_slack_is_detected(log):
    """Regression test: the comparisson between slack and halo messasges 
    was failing. Slack uses the emoji name and halo using the emoji 
    character.

    This was causing message from halo to be repeatedly added to slack as 
    ...
    results = messages_for_slack(slack, halo)
