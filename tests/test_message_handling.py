...
from haloslackchat.message import handler
from haloslackchat.message import is_resolved
from haloslackchat.models import HaloSlackChat
from haloslackchat.message import IGNORED_SUBTYPES
...
@patch('haloslackchat.message.get_ticket')
@patch('haloslackchat.message.close_ticket')
@patch('haloslackchat.message.create_ticket')
@patch('haloslackchat.message.message_issue_halo_url')
def test_new_support_message_creates_ticket(
    ...
    """Test the path to creating a halo ticket from new message receipt.
    """
    ...
    assert HaloSlackChat.objects.count() == 0
    ...
    assert HaloSlackChat.objects.count() == 1
    assert len(HaloSlackChat.open_issues()) == 1

    # Verify what the stored issue should look like:
    issue = HaloSlackChat.get('C019JUGAGTS', '1597940362.013100')
    ...
    message_issue_halo_url.assert_called_with(
        slack_client,
        'https://z.e.n.d.e.s.k',
        '32',
        '1597940362.013100',
...
@patch('haloslackchat.message.add_comment')
@patch('haloslackchat.message.get_ticket')
@patch('haloslackchat.message.close_ticket')
@patch('haloslackchat.message.create_ticket')
@patch('haloslackchat.message.post_message')
@pytest.mark.parametrize(
    'resolve_command',
    [
        'resolve ticket',
...
def test_halo_comment_and_resolve_ticket_command_closes_the_issue(
    post_message,
    create_ticket,
    close_ticket,
    get_ticket,
    ...
    """Test a comment is sent to Halo and that a ticket can be resolved with
    the 'resolve ticket' message (not sent to Halo).
    ...
    # Verify what the stored issue should look like:
    issue = HaloSlackChat.get('C0192NP3TFG', '1602064330.001600')
    assert issue.active is True
    assert issue.opened is not None
    assert issue.closed is None
    ...
    # Check a new comment is sent over to halo:
    #
    ...
    # Check the comment was "sent" to Halo correctly:
    ...
    # There should now be one instance here:
    assert HaloSlackChat.objects.count() == 1
    assert len(HaloSlackChat.open_issues()) == 0
...
@patch('haloslackchat.message.create_ticket')
@patch('haloslackchat.message.post_message')
def test_message_with_existing_support_ticket_in_halo(
    post_message,
    create_ticket,
    close_ticket,
    get_ticket,
    ...
    """Test further in-thread messages don't result in new halo tickets.
    """
    slack_client = MagicMock()
    halo_client = MagicMock()
    workspace_uri = 'https://s.l.a.c.k'
    ...
    results = HaloSlackChat.open_issues()
...
@patch('haloslackchat.message.create_ticket')
@patch('haloslackchat.message.post_message')
def test_thread_message_with_support_ticket_in_halo(
    post_message,
    create_ticket,
    close_ticket,
    get_ticket,
    ...
    """Test in-thread conversation messages are shipped to Halo.
    """
    slack_client = MagicMock()
    halo_client = MagicMock()
    workspace_uri = 'https://s.l.a.c.k'
    ...
    # should happen here is the comment gets shipped to Halo

    # called with the content of data['user']
    slack_client.users_info.assert_called_with(user='UGF7MRWMS')

    # Check the ticket is "recovered" and the comment is "added" to it:
    get_ticket.assert_called_with(halo_client, '83')
    add_comment.assert_called_with(
        halo_client,
        ticket,
...
@patch('haloslackchat.message.create_ticket')
@patch('haloslackchat.message.post_message')
def test_old_message_thread_with_message_and_no_support_ticket_in_halo(
    post_message,
    create_ticket,
    close_ticket,
    get_ticket,
    ...
    When no ticket is found in halo and ts & thread_ts are set, This
