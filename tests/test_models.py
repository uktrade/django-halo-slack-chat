...
@patch('haloslackchat.models.post_message')
...
def test_basic_cru_functionality(log, db):
    ...
    HaloSlackChat.open(
        ...
        ticket_id="halo-ticket-id-2",
        opened=datetime.datetime(2020, 7, 17, 14, 0, tzinfo=UTC)
    )

    assert HaloSlackChat.objects.count() == 2

    chat1 = HaloSlackChat.get("slack-channel-id-1", "slack-chat-id-1")
    ...
    chat2 = HaloSlackChat.get("slack-channel-id-2", "slack-chat-id-2")
...
def test_not_found(db):
    ...
    with pytest.raises(NotFoundError):
        HaloSlackChat.get("slack-channel-id-1", "slack-chat-id-1")

    with pytest.raises(NotFoundError):
        HaloSlackChat.get_by_ticket("slack-chat-id-1", "halo-ticket-id-1")

    with pytest.raises(NotFoundError):
        HaloSlackChat.resolve("slack-channel-id-1", "slack-chat-id-1")


def test_no_open_issues(db):
    """Verify I get no open issues when DB is empty.
    """
    assert HaloSlackChat.open_issues() == []
